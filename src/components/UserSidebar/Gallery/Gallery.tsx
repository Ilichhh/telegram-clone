...
function Gallery() {
  ...
  const { setUrl, setImagePopup } = useContext(ModalPhotoContext);
  const [imagesArray, setImagesArray] = useState([]);

  const openPopup = (currentImgUrl: string) => {
    ...
    if (activeChatID) {
      onSnapshot(doc(db, 'chats', activeChatID), (d) => {
        ...
        setImagesArray(data?.messages.map((message: DocumentData) => (
          message.imageUrl && <button type="button" key={message.id} onClick={() => openPopup(message.imageUrl)} className="btn"><img key={message.id} className="img" src={message.imageUrl} alt="Media in chat" /></button>
        )) || []);
      });
    }
  }, [activeChatID]);
